...
prop_traverseDirectory = testWarden $ do
  fs <- directoryFiles <$> traverseDirectory (MaxDepth 4) [] "test/data/view/view"
...
prop_traverseView' = testWarden $ do
  (bads, goods) <- traverseView' (View "test/data/view/view")
...
prop_traverseView_empty = testIO $ do
  r <- runEitherT . mapEitherT runResourceT $ traverseView (View "test/data/view/empty-view")
  pure $ r === (Left $ WardenTraversalError EmptyView)

prop_traverseView_bad :: Property
prop_traverseView_bad = testIO $ do
  r <- runEitherT . mapEitherT runResourceT $ traverseView (View "test/data/view/view")
...
prop_traverseView_deep = testIO $ do
  r <- runEitherT . mapEitherT runResourceT $ traverseView (View "test/data/view/deep-view")
  pure $ r === (Left $ WardenTraversalError MaxDepthExceeded)

prop_traverseView_good :: Property
prop_traverseView_good = testIO $ do
  r <- runEitherT . mapEitherT runResourceT $ traverseView (View "test/data/view/good-view")
